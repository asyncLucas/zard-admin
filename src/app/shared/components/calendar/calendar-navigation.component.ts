...
@Component({
  ...
  imports: [TranslatePipe, ZardButtonComponent, ZardIconComponent, ZardSelectComponent, ZardSelectItemComponent],
  template: `
    <div [class]="navClasses()">
      <button z-button type="button" zType="ghost" zSize="sm" (click)="onPreviousClick()" [disabled]="isPreviousDisabled()" aria-label="Previous month" class="h-7 w-7 p-0">
      ...
      <div class="flex items-center space-x-2">
        <!-- Month Select -->
        <z-select class="min-w-20" [zValue]="currentMonth()" [zLabel]="currentMonthName() | translate" (zSelectionChange)="onMonthChange($event)">
          @for (month of months; track $index) {
            <z-select-item [zValue]="$index.toString()">{{ month | translate }}</z-select-item>
          }
        </z-select>
      ...
      <button z-button type="button" zType="ghost" zSize="sm" (click)="onNextClick()" [disabled]="isNextDisabled()" aria-label="Next month" class="h-7 w-7 p-0">
...
export class ZardCalendarNavigationComponent {
  ...
  readonly months = this.#dateService.months();
