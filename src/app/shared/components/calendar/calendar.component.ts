...
export class ZardCalendarComponent {
  ...
  protected readonly currentMonthValue = linkedSignal(() => {
    ...
    return date instanceof Date && !isNaN(date.getTime()) ? date.getMonth().toString() : new Date().getMonth().toString();
  });
  protected readonly currentYearValue = linkedSignal(() => {
    const date = this.currentDate();
    return date instanceof Date && !isNaN(date.getTime()) ? date.getFullYear().toString() : new Date().getFullYear().toString();
  });
  ...
  protected previousMonth(): void {
    ...
    const currentYear = Number.parseInt(this.currentYearValue());
    const previous = new Date(
      Number.isNaN(currentYear) ? currentDate.getFullYear() : currentYear,
